...
# Create IPC directory: sticky + world-write so the app (user) can drop a
# trigger. Auth is trigger-ownership (allowed_uid), not directory membership.
mkdir -p "${IPC_DIR}"
chown root:wheel "${IPC_DIR}"
chmod 1733 "${IPC_DIR}"

# Pin the account allowed to send commands (active console user at install).
CONSOLE_UID=$(/usr/bin/stat -f%u /dev/console 2>/dev/null || echo 501)
# loginwindow owns /dev/console before first login — fall back to the invoking user.
if [ "${CONSOLE_UID}" -lt 501 ] 2>/dev/null; then
    CONSOLE_UID="${SUDO_UID:-501}"
fi
echo "${CONSOLE_UID}" > "${ALLOWED_UID_FILE}"
chown root:wheel "${ALLOWED_UID_FILE}"
chmod 600 "${ALLOWED_UID_FILE}"
info "Allowed IPC UID: ${CONSOLE_UID}"
