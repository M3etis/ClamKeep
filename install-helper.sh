...
if [ "$EUID" -ne 0 ]; then
    echo -e "${RED}[ERROR]${NC} Запустите с sudo: sudo ./install-helper.sh"
    exit 1
fi

info "Установка helper daemon..."
...
<dict>
    ...
    <string>/tmp/clamkeep-helper.log</string>
...
info "Daemon установлен и запущен!"
info "Для удаления: sudo launchctl bootout system/com.m3etis.clamkeep.helper"
